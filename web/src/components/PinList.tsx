      ...
      <List dense={true}>
        {board.pins.map((pin) => (
          <>
            <ListItem key={pin.id}>
              <ListItemIcon>
                {pin.type === PinType.Digital ? <PiWaveSquareLight size="20px" /> : <PiWaveSineLight size="20px" />}
              </ListItemIcon>
              <ListItemText primary={pin.id} secondary={pin.type} />
              <Switch checked={pin.mode === PinMode.Output} onChange={() => handleSetupPin(pin)} />
              <Switch checked={pin.state === pin.max} onChange={() => handleDigitalWrite(pin)} />
            </ListItem>
            <Divider />
          </>
        ))}
